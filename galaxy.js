(() => {
  ...
  const isMobile = navigator.maxTouchPoints > 0 && innerWidth < 900;

  function resize() {
    dpr = isMobile ? 1 : Math.min(devicePixelRatio || 1, 2);
    w = innerWidth;
    h = innerHeight;
    canvas.width = w * dpr;
    canvas.height = h * dpr;
  ...
  function makeSpace() {
    for (let i = 0; i < (isMobile ? 700 : 1750); i++) {
      const arm = i % 4,
    ...
    for (let i = 0; i < (isMobile ? 100 : 360); i++) {
  ...
  function rose(x, y, r, rot, alpha, kind) {
    ...
    ctx.shadowBlur = isMobile ? 0 : r * 0.78;
    const stem = () => {
  ...
  function drawHeart(x, y, r, label, t) {
    ...
    ctx.translate(x, y);
    ctx.shadowColor = cfg.glow;
    ctx.shadowBlur = isMobile ? 0 : r * 0.2;
    const heartDots = isMobile ? 60 : 150;
    for (let layer = 0; layer < (isMobile ? 1 : 3); layer++) {
      for (let i = 0; i < heartDots; i++) {
        const a = (i / heartDots) * Math.PI * 2,
          hx = 16 * Math.sin(a) ** 3,
          hy =
            13 * Math.cos(a) -
            5 * Math.cos(2 * a) -
  ...
  let lastFrame = 0;
  function frame(t) {
    if (isMobile && t - lastFrame < 25) { requestAnimationFrame(frame); return; }
    lastFrame = t;
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    ...
    for (const { o, p } of sorted) {
      ...
      ctx.globalAlpha = alpha;
      ctx.shadowColor = cfg.glow;
      ctx.shadowBlur = isMobile ? 0 : r * 0.6;
  ...
  const audioEl = new Audio("assets/Christina Perri - A Thousand Years.mp3");
  audioEl.loop = true;
  audioEl.volume = 0.55;
  function startMusic() {
    audioEl.play();
